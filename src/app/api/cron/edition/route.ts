  ...
  try {
    ...
    if (
      ...
      const timeSinceLastGeneration =
        (currentTime - editor.lastEditionGenerationTime) / (1000 * 60); // Convert to minutes
    ...
    console.log(
      `[${new Date().toISOString()}] Set newspaper job running=true and last_run=${currentTime}`
    );
    ...
    try {
      ...
      console.log(
        `[${new Date().toISOString()}] Set newspaper job running=false and last_success=${currentTime}`
      );

      console.log(
        `[${new Date().toISOString()}] Successfully generated hourly edition ${hourlyEdition.id}`
      );
      ...
      await redis.setJobRunning("newspaper", false);
      console.log(
        `[${new Date().toISOString()}] Set newspaper job running=false due to error`
      );
