...
export class SchedulerService {
  ...
  start(): void {
    console.log("Starting Newsroom Scheduler...");
    ...
    const reporterJob = new CronJob("*/15 * * * *", async () => {
      ...
      console.log(
        `[${new Date().toISOString()}] Starting scheduled article generation...`
      );
      ...
      try {
        ...
        console.error(
          `[${new Date().toISOString()}] Reporter job failed:`,
          error
        );
      }
    });
    ...
    const newspaperJob = new CronJob("0 */3 * * *", async () => {
      ...
      console.log(
        `[${new Date().toISOString()}] Starting scheduled newspaper edition generation...`
      );

      try {
        const edition = await this.editorService.generateHourlyEdition();
        console.log(
          `[${new Date().toISOString()}] Successfully generated newspaper edition: ${edition.id}`
        );
        console.log(
          `[${new Date().toISOString()}] Edition contains ${edition.stories.length} stories`
        );
        console.log("Newspaper edition job completed successfully\n");
      } catch (error) {
        console.error(
          `[${new Date().toISOString()}] Newspaper edition job failed:`,
          error
        );
      }
    });
    ...
    const dailyJob = new CronJob("0 0 * * *", async () => {
      ...
      console.log(
        `[${new Date().toISOString()}] Starting scheduled daily edition generation...`
      );
      ...
      try {
        ...
        console.log(
          `[${new Date().toISOString()}] Daily edition contains ${dailyEdition.editions.length} newspaper editions`
        );
        console.log("Daily edition job completed successfully\n");
      } catch (error) {
        console.error(
          `[${new Date().toISOString()}] Daily edition job failed:`,
          error
        );
      }
    });
  ...
  stop(): void {
    console.log("Stopping Newsroom Scheduler...");
  ...
  async triggerNewspaperJob(): Promise<void> {
    ...
    try {
      const edition = await this.editorService.generateHourlyEdition();
      console.log(
        `Manually generated newspaper edition: ${edition.id} with ${edition.stories.length} stories`
      );
    } catch (error) {
  ...
  async triggerDailyJob(): Promise<void> {
    ...
    try {
      const dailyEdition = await this.editorService.generateDailyEdition();
      console.log(
        `Manually generated daily edition: ${dailyEdition.id} with ${dailyEdition.editions.length} editions`
      );
    } catch (error) {
