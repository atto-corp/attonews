...
async function migrateData() {
  ...
  try {
    ...
    console.log(`   Reporters: Redis=${reporters.length}, PostgreSQL=${postgresReporters.length}`);
    console.log(`   Articles: Redis=${articleCount}, PostgreSQL=${postgresArticles.length}`);
    console.log(`   Users: Redis=${users.length}, PostgreSQL=${postgresUsers.length}`);

    if (reporters.length === postgresReporters.length &&
